/**
 * auth.js — Autenticación ONE Platform DISC
 * Porta la misma lógica de Eneagrama:
 * - Login USER: itera todos los admins hasta encontrar el usuario
 * - Guarda apiUsuarios / apiRespuestas / apiVisualizacion en sessionStorage
 */
...
const Auth = {
    ...
    try {
      ...
      if (rol === CONFIG.roles.ADMIN) {
        ...
        this.setSession({
          ...
          nombreEmpresa:    String(adminEncontrado.Name_Empresa     || adminEncontrado.Empresa_Nombre || '').trim()
        });
        return { success: true };
      }

      // ─── 3. USER — itera todos los admins hasta encontrarlo ────
      if (rol === CONFIG.roles.USER) {
        try {
          ...
          this.setSession({
            ...
            nombreEmpresa:    String(adminDueño.Name_Empresa     || adminDueño.Empresa_Nombre || '').trim()
          });
          return { success: true };
  ...
  redirectToDashboard() {
    const session = this.getSession();
    if (!session) { window.location.href = CONFIG.routes.login; return; }
